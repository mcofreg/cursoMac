...
COLS = ["nombre_empresa", "razon_social", "rut", "confianza_rut", "telefono", "email",
...
for a in A:
    filas.append({**{c: "" for c in COLS},
        ...
        "confianza_rut": "confirmado (publicado por la fuente)",
...
for b in B:
    filas.append({**{c: "" for c in COLS},
        ...
        "rut": b.get("rut", ""), "confianza_rut": b.get("confianza_rut", ""),
        "criterio_verificado": ("sin sitio web + redes + RUT probable"
                                if b.get("rut") else "sin sitio web + redes sociales confirmadas"),
        "origen": "OpenStreetMap" + (" + Registro de Empresas y Sociedades" if b.get("rut") else ""),
        "fuente": b["fuente"]})
...
redes = lambda f: f['instagram'] or f['facebook'] or f['tiktok']
print(f"  con redes sociales ...... {sum(1 for f in filas if redes(f))}")
print(f"  con RUT *y* redes ....... {sum(1 for f in filas if f['rut'] and redes(f))}")
print(f"-> {ruta}")
